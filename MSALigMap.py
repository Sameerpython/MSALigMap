...
for keys,vals in pdb_seq_dict.iteritems():
...
with open('trimmedfasta.fasta', 'w') as files:
    for seqids, seqn in combine.iteritems():
        files.write( ">" + seqids)
        files.write("\n")
        files.write (seqn[0])
        files.write("\n")
...
for ids,lig in pdbid_lig.iteritems():
...
for (lurl,murl), (lid,mid) in zip(pdbsum_dict.iteritems(), pdbid_lig.iteritems()):
...
H_printing = False
H_atoms_commoncomp={}
for pdbsumids, pdbsumlink in mydictcheck.iteritems():
...
for NONHpdbsumids,NONHpdbsumlink in mydictcheck.iteritems():
...
for ids,aa_numb in H_atoms_commoncomp.iteritems():
    for ids1, numb in pdb_seq_dict_numbering.iteritems():
        if ids==ids1:
...
for ids_1,aa_numb1 in NONHatoms_commoncomp.iteritems():
    for ids2, numb2 in pdb_seq_dict_numbering.iteritems():
                    ...
                    if dssplink==(check_ids.upper()):
                        ...
                        if dssp_href[0]=='<a href="entry?db=dssp&amp':
                            ...
                            for dssppagelines in dsspwebpagelink.iter_lines():
                                lines_in_dssp=str(dssppagelines).strip()
...
for  Hkey1,Hvalues1 in updated_indexed_bindingsite.iteritems():  
    H_count =0
    H_count1=0
    print "Hkey1",Hkey1
    Hkey1=str(Hkey1).split(':')[0]
    if aa_text_dict.has_key(Hkey1):
...
for  NONHkey1,NONHvalues1 in updated_indexed_NONHbindingsite.iteritems():  
    NONH_count =0
    NONH_count1=0
    NONHkey1=str(NONHkey1).split(':')[0]
    if aa_text_dict.has_key(NONHkey1):
...
with open('NEWstringmapped-all_1.html', 'w') as alignemnt:
        ...
        for dictkeys in sorted(Prot_AA.keys()):
            ...
            for i in Prot_AA[dictkeys][itemlength]:
                ...
                if i=='-':
                    ...
                    count+=1
                    gapcount+=1
                elif Alignment_adjusted_indexed_Hbindingsite.has_key(dictkeys) or Alignment_adjusted_indexed_NONHbindingsite.has_key(dictkeys):
                    if count in Alignment_adjusted_indexed_Hbindingsite[dictkeys]: #and not Alignment_adjusted_indexed_NONHbindingsite[dictkeys]:
                        ...
                        #resindex+=1
                        count+=1
                    #elif  count in   Alignment_adjusted_indexed_Hbindingsite[dictkeys] and Alignment_adjusted_indexed_NONHbindingsite[dictkeys]:
                     #   if i.startswith(('A','I','L','M','V')):
                      #      bold_underlined="<SPAN STYLE='text-decoration:overline; font-weight:bold; color:#FF1493'>%s</SPAN>" %i
                       #     alignemnt.write(bold_underlined)
                       # if i.startswith(('F','W', 'Y')):
                        #    bold_underlined="<SPAN STYLE='text-decoration:overline; font-weight:bold; color:#FF8C00'>%s</SPAN>" %i
                         #   alignemnt.write(bold_underlined) 
                       # if i.startswith(('K','R', 'H')):
                        #    bold_underlined="<SPAN STYLE='text-decoration:overline; font-weight:bold; color:red'>%s</SPAN>" %i
                         #   alignemnt.write(bold_underlined)
                       # if i.startswith(('E','D')):
                        #    bold_underlined="<SPAN STYLE='text-decoration:overline; font-weight:bold; color:#006400'>%s</SPAN>" %i
                         #   alignemnt.write(bold_underlined)
                        #if i.startswith(('N','Q','S','T')):
                         #   bold_underlined="<SPAN STYLE='text-decoration:overline; font-weight:bold; color:blue'>%s</SPAN>" %i
                          #  alignemnt.write(bold_underlined)
                       #if i.startswith(('G','P')):
                        #    bold_underlined="<SPAN STYLE='text-decoration:overline; font-weight:bold; color:#800080'>%s</SPAN>" %i
                         #   alignemnt.write(bold_underlined)
                        #if i.startswith(('C')):
                         #   bold_underlined="<SPAN STYLE='text-decoration:overline; font-weight:bold; color:yellow'>%s</SPAN>" %i
                          #  alignemnt.write(bold_underlined)
                        #resindex+=1
                        #count+=1
    ...
    mergeH_H_NHddict= {key: value + Alignment_adjusted_indexed_NONHbindingsite[key] for key, value in Alignment_adjusted_indexed_Hbindingsite.iteritems()}
    ...
    alignemnt.write("<div align='center'><b>Alignment and WebLogo of hydrogen bonded interacting residues</b></div>")
    alignemnt.write("<br/>")
    ...
    for H_webid in H_weblogo_gap_to_X.keys():
        ...
        H_protmotif.append(Seq(H_var, IUPAC.extended_protein))
    ...
    alignemnt.write("<div align='center'><b>Alignment and WebLogo of non-bonded interacting residues</b></div>")
    alignemnt.write("<br/>")
    ...
    for NH_webid in NH_weblogo_gap_to_X.keys():
        ...
        NH_protmotif.append(Seq(NH_var, IUPAC.extended_protein))
    ...
    alignemnt.write(NHsrc)
    alignemnt.write("<br/>")
    alignemnt.write("<br/>")

#alignemnt.write("Alignment of Both Hydrogen and Non-hydrogen interaction Residues")
    alignemnt.write("<div align='center'><b>Alignment and WebLogo of hydrogen and non-bonded interacting residues</b></div>")
    alignemnt.write("<br/>")
